...
const Comments = ({ postId }) => {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/comments?post_id=${postId}`);
  ...
  const handleCreateComment = async () => {
    try {
      const response = await axios.post('http://localhost:5000/api/comments', { post_id: postId, content: newComment }, {
  ...
  const handleUpdateComment = async (id, updatedContent) => {
    try {
      const response = await axios.put(`http://localhost:5000/api/comments/${id}`, { content: updatedContent }, {
        headers: { Authorization: `Bearer ${token}` }
      });
